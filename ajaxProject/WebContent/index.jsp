...
<body>
	...
	<p>
		...
		우리가 기존에 a 태그로 요청 및 form submit 요청 방식은 "동기식 요청" => 응답페이지가 돌아와야만 볼 수 있음 (페이지가 깜빡거림) <br><br>
		...
		* 동기식 / 비동기식 <br>
		...
		<ul>
			...
			<li>서버에 요청한 결과까지의 시간이 지연되면 무작정 계속 기다려야됨 (흰 페이지로 보여질것임)</li>
		...
		ex) 실시간 급상승 검색어 로딩, 검색어 자동완성, 아이디 중복체크, 찜하기/해제하기, 추천, 댓글, 무한스크롤링(페이지 대체) 등등..
		<br><br>

		* 비동기식의 단점 <br>
		- 현제 페이지에 지속적으로 리소스가 쌓임 => 페이지가 느려질 수 있음 <br>
		- 에러 발생시 디버깅 어려움 <br>
		- 요청 처리 후에 돌아온 응답데이터를 가지고 현재페이지에 새로운 요소를 만들어서 뿌려줘야될것임 => dom 요소들을 새로이 만들어내는 구문을 잘 익혀둬야함
		<br><br>
	...
	* 주요속성
	  ...
	  - data : 요청시 전달할 값
	  - success : ajax통신 '성공'했을 때 실행할 함수 정의
	  - error : ajax통신 '실패'했을 때 실행할 함수 정의
	  - complete : ajax통신 실패했든 성공했든 간에 '무조건 실행'할 함수 정의
	...
	<script>
		$(function(){
			$("#btn1").click(function(){
				...
				// 기존에 동기식 통신
				// localhost:8001/ajax/jqAjax1.do?input=안녕
				...
				$.ajax({
					...
					data:{input:$("#input1").val()},	// 보낼 데이터. '키:벨류' 'input:안녕' 세트로 보내야함
					type:"get",							// 요청 방식 지정

					// 응답 받아주기
					success:function(result){ 		 // 성공시, controller에서 보낸 responseData가 응답데이터가 자동으로 매개변수로 넘어옴
						console.log("ajax 통신 성공!!");
						console.log(result);
						$("#output1").text(result);  // == output1의 값을 result로 바꿔줘라
					},
					error:function(){
						console.log("ajax 통신 실패ㅜㅜ");
					},
					complete:function(){
						console.log("ajax 통신 성공여부와 관계없이 무조건 호출!");
					}

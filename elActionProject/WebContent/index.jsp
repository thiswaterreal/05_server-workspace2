...
<body>
	...
	<p>
		기존에 사용했던 표현식(출력식) &lt;%= name%&gt; 와 같이 (name : 변수명 또는 메소드)<br>
	...
	<!-- 
		* JSP를 이루는 구성인자
		...
		3. 액션태그 (표준액션태그, 커스텀액션태그) : xml 기술을 이용해서 기존의 jsp문법을 확장하는 기술을 제공하는 태그
			...
			>> 표준액션태그(Standard Action Tag) : JSP 페이지에서 바로 사용 (별도의 라이브러리 연동 필요X)
											   모든 태그명 앞에 jsp: 접두어 붙음
			
			>> 커스텀액션태그(Custom Action Tag) : JSP 페이지에서 바로 사용 불가능 (별도의 라이브러리 연동 필요)
	 										   모든 태그명 앞에 jsp: 외의 다른 접두어 붙음(종류 다양함)
